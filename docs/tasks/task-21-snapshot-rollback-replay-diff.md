...
4. 实现 replay runner、按 canonical tick 顺序定位首次 mismatch 与 diff。状态
   hash 不保证一旦不同就永不重合，因此不得使用会跳过早期 mismatch 的普通二分。
...
- field翻转被精确diff；
- native 与 WASM serial replay 一致；1/2/4/8 worker replay 矩阵由 Task
  22A/23 在真实 executor 接入后完成，并作为 Task 23 的强制回归复用本任务 corpus。

## 完成判定

完整状态L4通过。只保存body transform、丢cache或仅最终帧相同不算完成。

本任务不实现 job system，也不将 executor 内部队列/调度日志写入快照或
GRAVREPL。跨 worker 验收不能在 Task 22A 之前伪造。
