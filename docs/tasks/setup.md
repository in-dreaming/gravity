...
- job system 是 C ABI 的正式能力；宿主只提供同步 `dispatch_batch`
  callback，按逻辑 job index 执行并在返回前完成整批，禁止保存 descriptor、
  禁止重入 World；不暴露 executor queue、task handle 或 enqueue/wait 生命周期；
...
- 单线程实现永远保留为 oracle；
- 主线程先生成有序 work list；输出由逻辑 job/range index 拥有，不得以实际
  worker ID 决定容量、布局或失败；可变长度输出使用 count→canonical prefix
  sum→fill，或固定 input slot→canonical compact；
...
- dispatch/backpressure/worker fault 发生时不得发布部分 Tick：所有 fallible
  kernel 先写预分配 staging，barrier 成功后才由主线程 canonical commit；
...
- `zig build demo`：构建WASM + 以package/lock/Node/pnpm版本为内容戳增量执行
  `pnpm install --frozen-lockfile` + Vite production build到`zig-out/demo`；lock与
  工具版本未变时不得重复安装；
...
`third_party/spindle` 仅由 `jobs` 的 native adapter 通过公开
`src/executor.zig` / `spindle_executor` 窄入口使用。核心物理模块、state
codec、ABI、WASM serial build 不得直接 import Spindle；Gravity 不导入
Spindle aggregate Runtime、parallel helpers、Local Task Graph、ECS、Resource
Graph、Workflow、SQLite、archive、I/O 或 observability。物理 work partition、
staging、merge、fault publication 与 determinism 全部由 Gravity 拥有。
...
20+21+22 ─22A spindle-executor-adoption
22A      ─23 deterministic-job-system
20+21+23 ─24 optimization-benchmarks
05+21+23 ─25 fuzz-security-hardening
22    ─26 demo-wasm-typescript-build
26 ─27 demo-three-react-cases
22+22A+23+24+25+27 ─28 product-qualification

可选调研，不阻塞主链：
22A contract ─29 cloth research
22A contract ─30 soft-body research
22A contract ─31 deformable-mesh/fracture research
```
